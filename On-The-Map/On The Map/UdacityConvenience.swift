...
extension UdacityClient {
    ...
    func authenticateBasicLoginWithViewController(_ hostViewController: LoginViewController, completionHandler: (success: Bool, errorString: String?) -> Void) {
    ...
    func authenticateFacebookLoginWithViewController(_ hostViewController: LoginViewController, completionHandler: (success: Bool, errorString: String?) -> Void) {
        hostViewController.indicator(true)
        if FBSDKAccessToken.current() != nil {
            self.getSessionFacebookID(FBSDKAccessToken.current().tokenString){ result, errorString in
    ...
    func authenticateStudentLocationsWithViewController(_ hostViewController: UIViewController, completionHandler: (success: [Bool], errorString: String?) -> Void) {
    ...
    func getSessionID(_ username: String,password: String, completionHandler: (result: String?, error: NSError?) -> Void) {
        ...
        _ = taskForPOSTMethod(mutableMethod,parse:false, parameters: nil , jsonBody: jsonBody) { JSONResult, error in
            ...
            if let error = error {
                ...
                if let results = JSONResult?.value(forKey: UdacityClient.JSONResponseKeys.Session)?.value(forKey: UdacityClient.JSONResponseKeys.Id) as? String {
                    self.sessionID = results // Setting the session ID
                    if let key = JSONResult?.value(forKey: UdacityClient.JSONResponseKeys.Account)?.value(forKey: UdacityClient.JSONResponseKeys.Key) as? String{
                        self.uniqueKey = key
                    }
    ...
    func getSessionFacebookID(_ access_token: String, completionHandler: (result: String?, error: NSError?) -> Void) {
    ...
    func getPublicUserData(_ uniqueKey: String,completionHandler: (result: Student?, error: String?) -> Void) {
        /* 1. Specify parameters, method (if has {key}), and HTTP body (if POST) */
        ...
        taskForGETMethod(method,parse: false, parameters: parameters) { JSONResult, error in
            ...
            if let _ = error{
                ...
                if let lastname = JSONResult?.value(forKey: UdacityClient.JSONResponseKeys.User)?.value(forKey: UdacityClient.JSONResponseKeys.Last_Name) as? String {
                    if let firstname = JSONResult?.value(forKey: UdacityClient.JSONResponseKeys.User)?.value(forKey: UdacityClient.JSONResponseKeys.First_Name) as? String {
                        if let uniqueKey = JSONResult?.value(forKey: UdacityClient.JSONResponseKeys.User)?.value(forKey: UdacityClient.JSONResponseKeys.Key) as? String {
    ...
    func getStudentLocations(limit:Int, skip:Int,completionHandler: (result: [Student]?, error: String?) -> Void){
        ...
        taskForGETMethod(method,parse: true, parameters: nil) { JSONResult, error in
            ...
            if let _ = error{
                ...
                if let results = JSONResult?.value(forKey: UdacityClient.JSONResponseKeys.Results) as? [[String : AnyObject]] {
    ...
    func queryStudentLocation(_ uniqueKey: String,completionHandler: (result: Student?, error: String?) -> Void) {
        var parameters:[String:AnyObject] = [String:AnyObject]()
    ...
    func saveAccountLocation(_ account:Student,completionHandler: (result: Bool?, error: NSError?) -> Void) {
        ...
        _ = taskForPOSTMethod(method, parse: true,parameters: nil , jsonBody: jsonBody) { JSONResult, error in
            ...
            if let error = error {
                ...
                if let _ = JSONResult?.value(forKey: UdacityClient.JSONResponseKeys.ObjectID) as? String {
    ...
    func updateAccountLocation(_ account:Student,completionHandler: (result: Bool?, error: NSError?) -> Void) {
        ...
        _ = taskForPUTMethod(method,parameters: nil , jsonBody: jsonBody) { JSONResult, error in
            ...
            if let error = error {
                ...
                if let _ = JSONResult?.value(forKey: UdacityClient.JSONResponseKeys.UpdatedAt) as? String {
                    completionHandler(result: true, error: nil)
                } else {
